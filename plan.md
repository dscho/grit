...
- [~] Make all current `t3` family tests fully pass. Work one dependency group at a time,
  ...
  - Current focus: `t3206-range-diff.sh` (48 total, 38 passed, 10 failing after making
    `HEAD@{1}` fall back to the current branch reflog when the HEAD reflog is too short), next in the
