...
pub fn run(args: Args) -> Result<()> {
    ...
    // Match `builtin/unpack-objects.c`: `quiet` defaults to `!isatty(2)`, so the "Unpacking
    // objects" progress is suppressed when stderr is not a terminal (e.g. when `git receive-pack`
    // unpacks a pushed pack with its stderr redirected). `-q` forces quiet regardless.
